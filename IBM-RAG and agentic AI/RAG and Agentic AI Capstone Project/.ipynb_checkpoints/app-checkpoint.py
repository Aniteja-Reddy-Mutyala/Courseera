...
SYSTEM_PROMPT = """Your name is Connoisseur Companion. You are an expert agent in the food industry with over 15 years of experience delivering high quality and personalized recommendations to users. You have won many awards for this.

You have access to a database of California restaurants. You can use the following tools:
- get_restaurant_info: Look up specific restaurants by name
- recommend_by_vibe: Find restaurants matching a mood or atmosphere
- get_review: Retrieve detailed reviews of restaurants
"""

load_dotenv()

GROQ_API_KEY = os.environ.get("GROQ_API_KEY")  # ✅ renamed from project_id
...
async def chat_with_agent(user_message: str, history: list) -> str:
    """Connect to the MCP server, discover tools, run ReAct loop. The LLM decides 
    which tool to call, calls them via MCP server, and repeats until final text response."""
    ...
    async with Client(transport) as client:
        ...
        for _ in range(10):
            ...
            for tool_call in response.tool_calls:
                ...
                messages.append(ToolMessage(  # ✅ fixed - was missing ToolMessage()
                    content=tool_output,
