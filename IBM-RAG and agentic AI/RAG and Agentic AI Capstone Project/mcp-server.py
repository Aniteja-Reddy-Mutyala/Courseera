...
def get_review(restaurant_name: str) -> str:
    ...
    restaurants = load_restaurant_data()  # ✅ need restaurant data to match name → itemId
    query = restaurant_name.lower().strip()
    
    # ✅ Step 1: Find itemId from restaurant name
    matched_item_id = None
    for restaurant in restaurants:
        if query in restaurant.get("name", "").lower():
            matched_item_id = restaurant.get("itemId")
            break
    
    if not matched_item_id:
        return json.dumps({
            "status": "not_found",
            "message": f"No restaurant found for '{restaurant_name}'"
        }, indent=2)
    
    # ✅ Step 2: Find review by itemId
    matching_review = None
    for review in reviews:
        if review.get("itemId") == matched_item_id:
    ...
    return json.dumps({
        ...
        "title": matching_review.get("title", "N/A"),
        "reviewer": matching_review.get("userId", "N/A"),
        "rating": matching_review.get("rating", "N/A"),
        "review_text": matching_review.get("text", "N/A"),  # ✅ "text" not "review_text"
        "date": matching_review.get("date", "N/A"),         # ✅ "date" not "visit_date"
